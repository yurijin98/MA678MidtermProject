...
I conducted a multilevel linear regression model to find the relationship between the used car price and used car specification. In the process of modelling, I check the marginal Model Plots of predictor and added polynomial form on one predictor. After that, I used manufacture brand and body type as group-level predictors to construct varying-intercept model. The residuals of the fitted model don't have obvious correlation with varibales but have heteroskedasticity and don't follow normal distribution. The coefficients of the fitted model shows valid interpretation.
...
In this report, I conducted a multilevel linear regression model to analyse how these factors influence the used car price. The analysis consists of three sections. In Method section, I will first introduce the data source used in this report and conduct some data processing. Then I will conduct exploratory data analysis and fit the model. After that, I will check the residuals of the fitted model. In Result section, I will interpret the coefficient of fitted model and display the random effect of multilevel model. In Discussion section, I will summarize what I have done and found in this report. Also, I will include limitation and next step work at last.  
...
The data is from [Kaggle-usedcarscatalog](https://www.kaggle.com/lepchenkov/usedcarscatalog). The following table is the explanation of some columns. 
...
The data has 38531 observations and 30 columns. In the following analysis, I choose 10 manufacture brands from Japan and Korea and several variables to conduct the analysis. I also conduct some transformation on some columns. For `year produced`, I use $year\_duration=log(2020-year\_produced)$ to get the age of cars being used. For `feature 0-9`, they are 9 bool type variables. I add them together as $feature=\sum_{i=0}^{9} feature_i$ to get the sum of feature.
...
Under the fitted multilevel model, for every one unit increase in feature, the average car price is expected to increase by $2.7\%$ when other variables remained unchanged. For every 100000 kilometers increase in odometer value, the average car price is expected to reduce by $3.4\%$ when other variables remained unchanged. For every one unit increase in engine capacity, the average car price is expected to increase by $28.9\%$ when other variables remained unchanged. For cars with warranty, the average car price is expected to increase by $8.5\%$ compared with those without warranty when other variables remained unchanged. For year duration, it is hard to directly interpret in that this term include polynomial degree. Here I use derivative instead to illustrate it. When we control other variables, We have:
...
The random effect plot shows that the car price vary from manufacturer and body type. Take Toyota as an example, most of their cars have positive random effect, but body types like cabriolet, suv and minibus enjoy a rather high random effect compared with other types. We can also compared the same body type with different manufacturers. For example, Toyota's suv has the highest random effect among all manufacturers' suv. 
...
Third, the residuals in the fitted model don't follow normal distribution and have heteroskedasticity. Therefore, the residuals don't completely confirms to the assumptions based on linear regression. 
Fourth, the overall used car market price may change over time and the market price may be affected by macro index as well. This report don't take these factors into consideration. 
